...
pub fn solve_basic_fish(sudoku: &SudokuSolver) -> Option<Step> {
    for size in 2..=4 {
        for value in 1..=9 {
            return_if_some!(simple_fish::search_simple_fish(sudoku, size, value, StepRule::BasicFish));
        }
    }
    None
}
...
pub fn solve_finned_fish(sudoku: &SudokuSolver) -> Option<Step> {
    for size in 2..=4 {
        for value in 1..=9 {
            return_if_some!(simple_fish::search_simple_fish(sudoku, size, value, StepRule::FinnedFish));
        }
    }
    None
}

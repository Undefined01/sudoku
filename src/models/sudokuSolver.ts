...
export class SudokuSolver {
    ...
    this.foreachHouse((cells) => {
      for (const cell1 of cells) {
        for (const cell2 of cells) {
          ...
          if (
            this.sudoku.getCell(cell1).value === undefined ||
            this.sudoku.getCell(cell2).value === undefined
          ) {
            continue;
          }
          if (
            this.sudoku.getCell(cell1).value ===
            this.sudoku.getCell(cell2).value
          ) {
            isValid = false;
            return stopSymbol;
          }
  ...
  fillPencilMarks() {
    ...
    this.foreachHouse((cells) => {
      const values = cells
        .values()
        .map((cell) => this.sudoku.getCell(cell).value);
  ...
  solveOneStep() {
    const solvers = [
      this.fullHouse,
      this.nakedSingle,
      this.solveHiddenSingle,
      this.LockedCandidatesType,
      this.hiddenSubset,
      this.nakedSubset,
      this.basicFish,
      this.finnedFish,
  ...
  fullHouse(state: SudokuState) {
    this.foreachHouse((cells) => {
      const unfilledCells = cells
        .values()
        .filter((cell) => state.getCell(cell).value === undefined);
      if (unfilledCells.length === 1) {
        ...
        const missingValue =
          ...
          cells
            .values()
            .reduce((acc, cell) => acc + (state.getCell(cell).value || 0), 0);
        this.setCellValue(state, cell, missingValue);
        console.log(
          `Full house: in ${cells.name}, there is only one blank => r${cell.row + 1}c${cell.column + 1}=${missingValue}`,
        );
        return stopSymbol;
      }
    });
    ...
    this.foreachHouse((cells) => {
      for (const cell of cells) {
        ...
        if (pencilMarks.size === 1) {
          ...
          console.log(
            `Naked single: there is only one candidate for r${cell.row + 1}c${cell.column + 1} => r${cell.row + 1}c${cell.column + 1}=${value}`,
          );
  ...
  solveHiddenSingle(state: SudokuState) {
    this.foreachHouse((cells) => {
      for (let value = 1; value <= 9; value++) {
        const cellsWithPencilMark = this.possibleCellsForCandidate
          .get(value)
          ?.intersection(cells);
        if (cellsWithPencilMark?.size === 1) {
          const targetCell = state.getCell(
            [...cellsWithPencilMark][0],
          ).position;
          console.log(
            `Hidden single: in ${cells.name}, there is only one possible cell that can fill ${value} => r${targetCell.row + 1}c${targetCell.column + 1}=${value}`,
          );
  ...
  LockedCandidatesType(state: SudokuState) {
    const check = (houseA: CellSet, houseB: CellSet) => {
      ...
      for (let value = 1; value <= 9; value++) {
        const possibleCellsInA = houseA
          .values()
          .filter((cell) => state.getCell(cell).pencilMarks.has(value));
        ...
        if (possibleCellsInA.every((cell) => intersection.has(cell))) {
          ...
          for (const cell of houseB.values()) {
            if (
              !intersection.has(cell) &&
              state.getCell(cell).pencilMarks.has(value)
            ) {
              this.deleteCandidate(state, cell, value);
              console.log(
                `Locked candidates: in ${houseA.name}, ${value} must be filled in one of ${intersection} => r${cell.row + 1}c${cell.column + 1}<>${value}`,
              );
  ...
  hiddenSubset(state: SudokuState) {
    this.foreachHouse((cells) => {
      ...
      for (let value = 1; value <= 9; value++) {
        const possibleCells = this.possibleCellsForCandidate
          .get(value)
          ?.intersection(cells);
      ...
      for (let size = 2; size <= 4; size++) {
        const possibleHouseCellsForCandidateInSize = new Array<
          [number, CellSet]
        >();
        for (let value = 1; value <= 9; value++) {
          const possibleCells = possibleHouseCellsForCandidate.get(value);
          if (possibleCells && possibleCells.size <= size) {
            possibleHouseCellsForCandidateInSize.push([
              value,
              possibleHouseCellsForCandidate.get(value)!,
            ]);
          }
        }
        ...
        for (const subset of SudokuSolver.permutations(
          possibleHouseCellsForCandidateInSize,
          size,
        )) {
          ...
          if (cellUnion.size <= size) {
            ...
            for (const cell of cellUnion) {
              for (let value = 1; value <= 9; value++) {
                if (
                  !valuesInSubset.has(value) &&
                  state.getCell(cell).pencilMarks.has(value)
                ) {
                  this.deleteCandidate(state, cell, value);
                  candidateDeleted = true;
                  console.log(
                    `Hidden subset: in ${cells.name}, candidates ${[...valuesInSubset]} only appears in ${cellUnion} => r${cell.row + 1}c${cell.column + 1}<>${value}`,
                  );
                }
              }
            }
  ...
  nakedSubset(state: SudokuState) {
    const check = (cells: CellSet, size: number) => {
      ...
      for (const subset of SudokuSolver.permutations(possibleValues, size)) {
        const valueUnion = subset.reduce((acc, [_, candidates]) => {
          candidates.forEach((n) => acc.add(n));
          return acc;
        }, new Set<number>());
        const cellsInSubset = CellSet.fromPositions(
          subset.map(([cell, _]) => cell),
        );
          ...
          for (const cell of cells) {
            ...
            for (const value of valueUnion) {
              if (this.candidatesForCell.get(cell.idx)?.has(value)) {
                this.deleteCandidate(state, cell, value);
                console.log(
                  `Naked subset: in ${cells.name}, ${cellsInSubset} only contains ${[...valueUnion]} => r${cell.row + 1}c${cell.column + 1}<>${value}`,
                );
                changed = true;
              }
            }
            ...
            for (const cell of otherCells) {
              ...
              for (const value of valueUnion) {
                if (this.candidatesForCell.get(cell.idx)?.has(value)) {
                  this.deleteCandidate(state, cell, value);
                  console.log(
                    `Locked subset: in ${cells.name}, cell ${cellsInSubset} only contains ${[...valueUnion]} => r${cell.row + 1}c${cell.column + 1}<>${value}`,
                  );
                  changed = true;
                }
              }
  ...
  basicFish(state: SudokuState) {
    const fishNames: { [keys: number]: string } = {
      2: "X-Wing",
      3: "Swordfish",
      4: "Jellyfish",
    };
    ...
    for (let size = 2; size <= 4; size++) {
      for (let value = 1; value <= 9; value++) {
        ...
        for (let i = 0; i < 9; i++) {
          const possibleCellsInRow = this.possibleCellsForCandidate
            .get(value)
            ?.intersection(this.cellsInRows[i]);
          if (
            possibleCellsInRow &&
            possibleCellsInRow.size > 0 &&
            possibleCellsInRow.size <= size
          ) {
            possibleCellsInRow.name = this.cellsInRows[i].name;
            possibleCellsInRows.push(possibleCellsInRow);
          }
          const possibleCellsInColumn = this.possibleCellsForCandidate
            .get(value)
            ?.intersection(this.cellsInColumns[i]);
          if (
            possibleCellsInColumn &&
            possibleCellsInColumn.size > 0 &&
            possibleCellsInColumn.size <= size
          ) {
        ...
        const check = (baseSet: CellSet[], coverSet: CellSet[]) => {
          ...
          for (const cell of coverCells) {
            ...
            this.deleteCandidate(state, cell, value);
            console.log(
              `${fishNames[size]}: for ${value}, ${baseSet.map((s) => s.name)} is covered by ${coverSet.map((s) => s.name)} => ${cell}<>${value}`,
            );
            changed = true;
          }
          return changed;
        ...
        for (const colSet of SudokuSolver.permutations(
          possibleCellsInColumns,
          size,
        )) {
          for (const rowSet of SudokuSolver.permutations(
            possibleCellsInRows,
            size,
          )) {
    ...
    for (let size = 2; size <= 4; size++) {
      for (let value = 1; value <= 9; value++) {
        ...
        for (let i = 0; i < 9; i++) {
          ...
          const possibleCellsInBlock = this.possibleCellsForCandidate
            .get(value)
            ?.intersection(this.cellsInBlocks[i]);
        ...
        const check = (baseSet: CellSet[], coverSet: CellSet[]) => {
          ...
          for (const cell of coverCells) {
            ...
            const canBeEliminated = allOf(fins, (fin) =>
              this.constrainsOfCell
                .get(fin.idx)!
                .some((constraint) => constraint.has(cell)),
            );
            if (!canBeEliminated) {
              continue;
            }
            ...
            console.log(
              `${hasFin ? "Finned" : "Complex"} ${fishNames[size]}: for ${value}, ${baseSet.map((s) => s.name)} is covered by ${coverSet.map((s) => s.name)}${hasFin ? ` with fin ${fins}` : ""} => ${cell}<>${value}`,
            );
            changed = true;
          }
          return changed;
        ...
        if (!searchComplexFish) {
          ...
          for (const colSet of SudokuSolver.permutations(
            [...possibleCellsInColumns, ...possibleCellsInBlocks],
            size,
          )) {
            // 两两之间互不相交
            if (
              [...SudokuSolver.permutations(colSet, 2)].some(
                (set) => !CellSet.intersection(...set).isEmpty(),
              )
            ) {
              continue;
            }
          ...
          for (const rowSet of SudokuSolver.permutations(
            [...possibleCellsInRows, ...possibleCellsInBlocks],
            size,
          )) {
            // 两两之间互不相交
            if (
              [...SudokuSolver.permutations(rowSet, 2)].some(
                (set) => !CellSet.intersection(...set).isEmpty(),
              )
            ) {
              continue;
